...
class LogisticRegression_ADA(object):
    ...
    def __init__(seld, eta=1, n_iter=100, random_state=1, alpha=0, shuffle=False):
        self.eta = eta
        self.n_iter = n_iter
        self.random_state = random_state    # seed number for randomize
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_iter):
            ...
            for xi, target in zip(X, y):    # iterate on single example
                cost = []    # record cost for each example
...
print("Save")
sample = pd.read.csv("sample_submission.csv")
sample["label"] = pred_y
