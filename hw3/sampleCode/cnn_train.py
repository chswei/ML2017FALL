# 不同於平常網路上的keras教學，
# 我們使用比較多manual的loop code來讓同學比較能瞭解"epoch" “batch” …的概念是什麼
...
# The parameter is used for early stopping
PATIENCE = 5 

def main():
    parser = argparse.ArgumentParser(prog='cnn_train.py')
    ...
    '''
    To begin with, you should first read your csv training file and cut them into 
    training set and validation set. Such as:
    ...
    # training data
    train_pixels = pickle.load('../train_pixels.pkl')
    train_labels = pickle.load('../train_labels.pkl')
    print('# of training instances: ' + str(len(train_labels)))

    # validation data
    valid_pixels = pickle.load('../valid_pixels.pkl')
    valid_labels = pickle.load('../valid_labels.pkl')
    print ('# of validation instances: ' + str(len(valid_labels)))

    '''
    Modify th answer format so as to correspond with the output of keras model.
    We can alse do this to training data here, but we choose to do it in "train" function.
    '''
    ...
    # start training
    train(args.batch, args.epoch, args.pretrain, args.save_every, train_pixels, train_labels, np.asarray(valid_pixels), np.asarray(valid_labels), args.model_name)
